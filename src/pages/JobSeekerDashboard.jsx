...
export default function JobSeekerDashboard() {
  ...
  const [profileData, setProfileData] = useState({
    name: localUser.first_name && localUser.last_name
      ...
      : localUser.first_name || localUser.last_name || localUser.name || localUser.email || "No Name",
    first_name: localUser.first_name || "",
    last_name: localUser.last_name || "",
    email: localUser.email || "",
    role: localUser.role || "Job Seeker",
    avatar: localUser.profile_picture || localUser.avatar || "https://imgs.search.brave.com/gXeZ5DUJxGMbr6_miepFf7Y2swQNqil9wU6LYVEacL8/rs:fit:500:0:1:0/g:ce/aHR0cHM6Ly9pbWdj/ZG4uc3RhYmxlZGlm/ZnVzaW9ud2ViLmNv/bS8yMDI0LzUvMTYv/ZTU0MDg5ZjMtZDdi/YS00OGZhLTg5Yzgt/ZTIwNDA3YTI3MTVk/LmpwZw",
  });

  const [openDropdown, setOpenDropdown] = useState(null);
  const [openSetting, setOpenSetting] = useState(null); // <-- Add this line
      ...
      try {
        ...
        setProfileData({
          name: user.first_name && user.last_name
            ...
            : user.first_name || user.last_name || user.name || user.email || "No Name",
          first_name: user.first_name || "",
          last_name: user.last_name || "",
          email: user.email || "",
          role: user.role || "Job Seeker",
          avatar: user.profile_picture || user.avatar || profileData.avatar,
        });
      } catch {
        setProfileData((prev) => ({
          ...prev,
          ...
          email: localUser.email || "",
          role: localUser.role || "Job Seeker",
          avatar: localUser.profile_picture || localUser.avatar || prev.avatar,
        }));
      }
    }
    fetchProfile();
  ...
  // Handler to toggle dropdown for a job
  const toggleDropdown = (jobId) => {
    setOpenDropdown(openDropdown === jobId ? null : jobId);
  };

  // Handler to toggle settings dropdown
            ...
            <div className="flex w-full flex-col gap-4 @[520px]:flex-row @[520px]:justify-between @[520px]:items-center">
              <div className="flex gap-4">
                ...
                <div className="flex flex-col justify-center">
                  <p className="text-[#0e1b13] text-[22px] font-bold leading-tight tracking-[-0.015em]">{profileData.name}</p>
                  <p className="text-[#4e976d] text-base font-normal leading-normal">{profileData.role}</p>
                  <p className="text-[#4e976d] text-base font-normal leading-normal">{profileData.joinDate}</p>
                </div>
              </div>
            </div>
          </div>
