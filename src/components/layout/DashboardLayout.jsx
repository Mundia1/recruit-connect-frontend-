    ...
    <>
      {navItems.map((item) => (
        <Link
          key={item.path}
          to={item.path}
          className={`group flex items-center rounded-md px-3 py-2 text-sm font-medium lg:px-4 ${isActive(
            item.path
          )}`}
          onClick={onItemClick}
        >
      ...
      <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
        <div className="flex min-h-0 flex-1 flex-col border-r border-gray-200 bg-white">
          ...
          <div className="flex flex-1 flex-col overflow-y-auto">
            <nav className="flex-1 space-y-1 py-4">{renderNavItems()}</nav>
          </div>
          <div className="flex flex-shrink-0 border-t border-gray-200 p-4">
            <div className="group block w-full">{renderUserProfile(true)}</div>
          </div>
        </div>
      </div>
